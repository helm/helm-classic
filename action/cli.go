...
// Cli return the cli application
func Cli() *cli.App {
	app := cli.NewApp()
	app.Name = "helm"
...
`
	...
	app.CommandNotFound = func(c *cli.Context, command string) {
		if HasPlugin(command) {
			Plugin(home(c), command, c.Args())
			return
		}
...
`,
			...
			Action: func(c *cli.Context) {
				...
				for _, chart := range c.Args() {
					Uninstall(chart, home(c), c.String("namespace"), c.Bool("force"))
				}
			},
			Flags: []cli.Flag{
				cli.StringFlag{
...
`,
			...
			Action: func(c *cli.Context) {
				minArgs(c, 1, "publish")
				Publish(c.Args()[0], home(c), c.String("repo"), c.Bool("force"))
			},
			Flags: []cli.Flag{
				cli.BoolFlag{
					Name:  "force, f",
...
`,
		...
		},
		{
			...
			Action: func(c *cli.Context) {
				minArgs(c, 1, "info")
				Info(c.Args()[0], home(c), c.String("format"))
			},
		},
		{
			Name:      "target",
...
func install(c *cli.Context) {
	...
	for _, chart := range c.Args() {
		Install(chart, h, c.String("namespace"), force, client)
	}
}

func searchCharts(c *cli.Context) {
	term := ""
	if len(c.Args()) > 0 {
		term = c.Args()[0]
	}
	Search(term, home(c), c.Bool("regexp"))
}
