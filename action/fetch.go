...
func fetch(chartName, lname, homedir, chartpath string) {
	...
	if err != nil {
		log.Warn("Oops. Look like there was an issue finding the chart, %s, n %s. Running the `helm updae` to ensure you have the latest version of all Charts from Github...", lname, src)
		Update(homedir)
		fi, err = os.Stat(src)
		if err != nil {
